...
from picture2word import picture2word_multi, picture2word_multi_
...
def test_vs (images_base,images_labels,images_test,imagestest_labels) :
    test_set_size = len(images_test)
    # Test uncompress
    pool = Pool()
    ...
    results = pool.starmap_async(picture2word_multi_, zip(images_test,imagestest_labels)).get()
    ...
    results = pool.starmap_async(picture2word_multi, zip(images_base,images_labels)).get()
    ...
    results = pool.starmap_async(picture2word_multi, zip(images_test,imagestest_labels)).get()
...
def multi_test (nb_run=100) :
    ...
    for i in range(len(Images_base)) :
        ...
        for j in range(len(Images_test)) :
            ...
            accuracy_tmp, accuracy_cp_tmp = test_vs(Images_base[i], Images_labels[i], Images_test[j], Images_test_labels[j])
